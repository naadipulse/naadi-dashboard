...
export default function LeftPanel({ mode = 'alliance' }) {
  ...
  const topParties = mode === 'individual' ? sortedParties : sortedParties.filter(p => p !== 'Others').slice(0, 4)
          ...
          {topParties.map((p) => {
            ...
            return (
              <div key={`won-${p}`} style={{
                ...
                <Photo photoUrl={photoUrl} fallback={cfg.short} color="#fff" size={mode === 'individual' ? 70 : 90} 
                  style={{ height: '100%', width: mode === 'individual' ? 70 : 85, zIndex: 1, objectFit: 'cover' }} />
